...
<template>
    <tr
        class="covidcostings-row"
        :class="{ 'is-not-pbo-costing': !costing.isPboCosting }"
        @click="log">
        <td>
            <div class="measure-name flex flex-row items-center">
                ...
                <div>
                    <component
                        ...
                        target="_blank"
                        :class="{
                            underline: currentCostingUrl,
                            'text-blue-700': currentCostingUrl,
                            'hover:text-blue-900': currentCostingUrl,
                        }"
                    ...
                    <div v-if="comparedCostingUrl">
                        <small class="archived">
                            <a
                                ...
                                class="font-thin"
                                >{{
                                    $root.strings.fullcostings.read_previous_pdf
                                }}</a
                            >
                        </small>
                    </div>
                ...
                <div
                    ...
                    <list-view-item-icon
                        type="down"
                        v-if="!displayWarning"
                        class="ml-2 h-4 w-6 rounded-sm bg-blue-100 px-1 hover:bg-blue-200"></list-view-item-icon>
                    <list-view-item-icon
                        type="up"
                        v-if="displayWarning"
                        class="ml-2 h-4 w-6 rounded-sm bg-blue-100 px-1 hover:bg-blue-200"></list-view-item-icon>
                ...
                <div
                    ...
                    <list-view-item-icon
                        type="note"
                        v-if="displayNote"
                        class="ml-2 h-4 w-6 rounded-sm bg-blue-800 px-1 text-white hover:bg-blue-700"></list-view-item-icon>
                </div>

                <template v-if="shouldShowStatusBadges">
                    <costing-badge v-if="costing.isNew">{{
                        $root.strings.fullcostings.new
                    }}</costing-badge>
                    <costing-badge v-if="costing.hasUpdatedArtifact">{{
                        $root.strings.fullcostings.updated_artifact
                    }}</costing-badge>
                    <costing-badge v-if="costing.hasUpdatedNumbers">{{
                        $root.strings.fullcostings.updated_numbers
                    }}</costing-badge>
                </template>
            </div>
            ...
            <transition name="fade">
                <qs-costings-note
                    v-if="displayWarning"
                    :note="warning"></qs-costings-note>
            </transition>
            <transition name="fade">
                <qs-costings-note
                    v-if="displayNote"
                    :note="note"></qs-costings-note>
            </transition>
        </td>
        ...
        computed: {
            shouldShowStatusBadges() {
                ...
                return (
                    ...
                    this.$root.currentUpdate.id ===
                        this.$root.currentGhostUpdateId
                );
            },
            ...
            currentCostingUpdate() {
                if (this.displayAsOfLatestScenario) {
                    return this.costing.updateWithId(
                        this.$root.currentUpdate.id,
                    );
                }
