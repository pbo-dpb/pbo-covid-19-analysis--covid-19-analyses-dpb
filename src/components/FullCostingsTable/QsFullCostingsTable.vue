...
<template>
    <div class="covidcostings">
        ...
        <template v-if="groups">
            ...
            <figcaption class="mb-8 border-t border-gray-300 pt-4">
                <markdown-content
                    :content="$root.strings.fullcostings.figcaption_line1"
                    class="prose-sm"></markdown-content>
                <markdown-content
    ...
    export default {
        ...
        computed: {
            ...
            noIncrementalImpactCostings() {
                return collect(this.$root.payload.costings).filter(
                    (costing) => {
            ...
            pendingCostings() {
                return collect(this.$root.payload.costings).reject(
                    (costing) => {
                        let costingDuringLastUpdate =
                            costing.wasPartOfScenarioWithId(
                                this.$root.currentUpdate.id,
                            );
                        ...
                        return (
                            costingDuringLastUpdate.id ===
                            costing.currentCostingUpdate.id
                        );
                    },
                ).items;
            },
