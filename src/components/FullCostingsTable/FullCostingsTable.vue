<template>
    <div>
        <notification-block
            ...
            type="warning"
            >{{
                $root.strings.fullcostings.archived.description
            }}</notification-block
        >
        ...
        <div>
            <header class="mb-4">
                <markdown-content
                    :content="
                        $root.strings.scenarioanalysis.descriptor
                    "></markdown-content>
            </header>
            <list-view
                v-if="scenarioItems"
                :items="scenarioItems"></list-view>
        </div>

        <hr class="my-4 border border-blue-800" />

        <header class="mb-4">
            <markdown-content
                :content="
                    $root.strings.fullcostings.descriptor
                "></markdown-content>
        </header>
        ...
        <div class="covidcostings">
            <qs-costings-loader
                v-if="
                    $root.payload && !$root.payload.updates
                "></qs-costings-loader>

            <template v-if="$root.payload && $root.payload.updates">
                <div class="flex w-auto flex-row justify-end">
                ...
                <qs-costings-search-results-table
                    v-else></qs-costings-search-results-table>
            </template>
        </div>
    </div>
</template>
