<template>
    <aside
        tabindex="-1"
        class="mx-2 my-4 rounded-lg border border-gray-100 px-2 py-4 text-sm shadow-lg">
        <h3
            class="mb-2 border-b border-gray-100 pb-2 text-center text-lg leading-none font-medium text-gray-800">
            {{ measure.title[language] }}
        </h3>

        <div
            v-for="note in measure.notes"
            :key="note.en"
            class="mb-2 rounded-sm p-2"
            :class="{
                'bg-blue-100': note.type == 'info',
                'bg-orange-100': note.type == 'warning',
            }">
            <h4 class="mb-2 text-left leading-none font-bold text-black">
                {{ $root.strings.costingerp.measures_notes.title }}
            </h4>

            <div>
                {{ note[language] }}
            </div>
        </div>

        <measures-list-item-details-costings
            :measure="measure"
            :highlighted-costing-id="highlightedCostingId"
            :prefer-net-cost="preferNetCost" />
    </aside>
</template>

<script>
    import MeasuresListItemDetailsCostings from "./MeasuresListItemDetailsCostings.vue";
    export default {
        components: {
            MeasuresListItemDetailsCostings,
        },
        computed: {
            language() {
                return this.language;
            },
        },
        props: {
            measure: Object,
            highlightedCostingId: String,
            preferNetCost: {
                type: Boolean,
                default: false,
            },
        },
    };
</script>
